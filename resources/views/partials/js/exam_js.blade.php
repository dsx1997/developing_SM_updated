    ...
    function stream_publish(exam_id,class_id) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.post('{{ route("stream_publish") }}',{exam_id:exam_id,class_id:class_id}, (res) => {
                    // window.location.href = 'http://localhost:8000/exams';
                })
            }
        })
                        ...
                        for (let ef of resp.examforms) {
                            if (entry.id == ef.exam_id) { // exams.id == examforms.exam_id
                                ...
                                if (flagexamname == false) {
                                    bb += `<td rowspan="` + rr + `" style='height:55px'>` + entry.name + `</td>`; //exams.name
                                    flagexamname = true;
                                }
                                for (let f of resp.forms) {
                                    if (f.id == ef.form_id) bb += `<td style='height:55px'>Form ` + f.name + `</td>`; // form.id == examforms.form_id
                                }
                                if (ef.flag) {
                                    bb += `<td style='height:55px'>Pending Publishing</td>`;
                                } else {
                                    bb += `<td style='height:55px'>Results Not Uploaded</td>`;
                                ...
                                if (flagexamedit == false) {
                                    bb += `<td class="d-none">Term` + entry.term + `</td>
                                            ...
                                            <td class="text-center" style='height:55px;padding:0px !important' rowspan="` + rr + `">
                                                <div class="col-2" display: flex;justify-content: space-around;>
                                                    <a class="btn d-flex flex-row" href='/exams/` + entry.id + `' style="color:#172b4c !important; background-color:#e6e6e6; border-color: #999; width: 100px;padding:6px;font-size:15px;">
                                ...
                                if (ef.flag) {
                                    bb += `<td class=action` + ef.exam_id + ef.form_id + ` style='height:55px;padding:5px !important' >
                                                <a class="btn d-flex flex-row"  style="color:#B5ACBC !important; background-color:#F9F9FB; pointer-events: none; cursor: default; border-color: #999; width: 140px;padding:15px;font-size:16px" disabled>
                                    ...
                                    bb += `<td class=action` + ef.exam_id + ef.form_id + ` style='height:55px;padding:5px !important'>
                                                <a class="btn d-flex flex-row" href="/exam_manage/config/` + ef.exam_id + `/` + ef.form_id + `" style="padding:8px;color:#172b4c !important; background-color:#e6e6e6; border-color: #999; width: 140px;padding:15px;font-size:16px">
                                                    <i _ngcontent-are-c166="">
                                ...
                                if (!ef.flag) {
                                    bb += `<td style='height:55px;padding:5px !important' class=action` + ef.exam_id + ef.form_id + `>
                                                        <a class="btn" href="/exam_manage/publish/` + ef.exam_id + `/` + ef.form_id + `"  style="font-size:16px;border-radius:7px;color:white; background-color:#152D4e; width: 132px; display: flex; flex-direction: row;padding:8px" >
                                    ...
                                    //                     </a>
                                    //         </td>`;
                                    bb += `<td class=action` + ef.exam_id + ef.form_id + ` style="padding:5px !important">
                                                <button type="button" class="btn" onclick="onDeleteEachExam(` + ef.exam_id + `, ` + ef.form_id + `, this);" style="border-radius:7px;margin-left:30px;font-size:16px;background: #ff562f; color: white; width: 100px; display: flex; flex-direction: row;padding:8px" >
                                    ...
                                    bb += `<td class=action` + ef.exam_id + ef.form_id + ` style="padding:5px !important">
                                                <a class="btn publish_btn" style="border-radius:7px;color:white; background-color:#00B6FF; width: 132px; display: flex; flex-direction: row;padding:5px 25px;font-size:16px">
                                                    <i _ngcontent-sdf-c166="" class="me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" style="margin-right:5px;margin-top:12px" class="bi bi-clipboard-data" viewBox="0 0 16 16">
                                    ...
                                    bb += `<td class=action` + ef.exam_id + ef.form_id + ` style="padding:5px !important">
                                                <button type="button" class="btn" onclick="SendRequest(` + ef.exam_id + `, ` + ef.form_id + `);" style="border-radius:7px;margin-left:20px;font-size:16px;background: white; color:#0092D3;border:1px solid #0092D3; width: 130px; display: flex; flex-direction: row;" >
                                                    <i _ngcontent-qlf-c166="" class="me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" style="margin-left:0px;margin-top:2px;margin-right:3px" class="bi bi-telegram" viewBox="0 0 16 16">
                                                            <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8.287 5.906c-.778.324-2.334.994-4.666 2.01-.378.15-.577.298-.595.442-.03.243.275.339.69.47l.175.055c.408.133.958.288 1.243.294.26.006.549-.1.868-.32 2.179-1.471 3.304-2.214 3.374-2.23.05-.012.12-.026.166.016.047.041.042.12.037.141-.03.129-1.227 1.241-1.846 1.817-.193.18-.33.307-.358.336a8.154 8.154 0 0 1-.188.186c-.38.366-.664.64.015 1.088.327.216.589.393.85.571.284.194.568.387.936.629.093.06.183.125.27.187.331.236.63.448.997.414.214-.02.435-.22.547-.82.265-1.417.786-4.486.906-5.751a1.426 1.426 0 0 0-.013-.315.337.337 0 0 0-.114-.217.526.526 0 0 0-.31-.093c-.3.005-.763.166-2.984 1.09z"/>
                                                        </svg>
                                                    </i>
                                                Send Request
                                                </button>
                                                <button type="button" class="btn" onclick="unpublish(` + ef.exam_id + `, ` + ef.form_id + `);" style="background-color: white; color: #CBAB89;border:1px solid #CBAB89;border-radius:8px;width: 170px;padding:10px 35px;font-size:16px;display: flex; flex-direction: row;" >
                                                    <i _ngcontent-qlf-c166="" class="me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-top:13px;float:left;" width="16" height="16" style="margin-right:2px" fill="currentColor" class="bi bi-box-arrow-in-down" viewBox="0 0 16 16">
    ...
    function selectExam() {
        ...
        req.done(function(res) {
            ...
            for (let my_class_sub of res.my_class_subject) {
                console.log("myclass_subject_array", my_class_sub)
                flag = 0;
                if ((res.types == "student" || res.types == "staff" || res.types == "teacher" || res.types == "admin")) {
                    ...
                    for (let exam_record of my_class_sub.subject.exam_record) {
                        if (exam_record.my_class_id == my_class_sub.my_class_id && exam_record.exam_id == exam.value && exam_record.af == my_class_sub.subject_id) {
                            ...
                            if (exam_record.ps == 0) {
                                flag = 1;
                                cc += `<td class="text-danger" style="font-family: 'Times New Roman', Times, serif;">Uploaded but not published by ` + res.teacher_name + `</td>`;
                            } else {
                                flag = 2;
                                cc += `<td class="text-success" style="font-family: 'Times New Roman', Times, serif;">Pending publishing by class by ` + res.teacher_name + `</td>`;
                            }
                            break;
                        }
